document.addEventListener("DOMContentLoaded",()=>{
    ...
    const username = userManager.getLoggedUser().username;
    ...
    function displayData(tasks){
        if(tasks.length === 0){
            ...
            taskWrapperEmpty.style.display = 'block';
        }else{
            taskWrapper.style.display = 'block';
            taskWrapperEmpty.style.display = 'none';

            taskWrapper.innerHTML = '';

            tasks.forEach((task)=>{
                const createdDate =  new Date(task.id).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
                const itemTask = document.createElement('div');
                itemTask.className = 'flex justify-between bg-white p-5 w-full rounded-3xl';
                itemTask.style.marginTop = '20px';
                itemTask.innerHTML = `
                <div class="task-card flex flex-col gap-5">
                    <div class="flex gap-3 items-center">
                        ...
                        <div class="flex flex-col">
                            <p class="font-bold text-lg leading-[27px]">${capitalizeFirst(task.taskName)}</p>
                            <p class="text-sm leading-[21px] text-taskia-grey">Created at ${createdDate}</p>
                        </div>
                    </div>
                    <div class="flex gap-4 font-semibold text-sm leading-[21px]">
                        ...
                        <div class="flex gap-1 items-center ${task.isCompleted ? 'text-taskia-green' : ''}">
                        <div class="flex shrink-0 w-5 h-5">
                        ...
                        <p>${task.isCompleted ? 'Completed' : 'In Progress'}</p>
                    </div>
                    </div>
                </div>
                <div class="flex flex-row items-center gap-x-3">
                    ...
                    <a href="#" id="completeTask-${task.id}" style="display : ${task.isCompleted ? 'none' : 'block'}" 
                ...
                itemTask.querySelector(`#completeTask-${task.id}`).addEventListener("click", ()=>{
                    taskManager.updateStatusComplete(task.id);
                    displayData(taskManager.getTasks());
                });
        
                itemTask.querySelector(`#deleteTask-${task.id}`).addEventListener("click", ()=>{
